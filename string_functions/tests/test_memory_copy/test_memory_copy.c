...
int main(int argc, char **argv)
{
    ...
    case to_chr:
        ...
        memory_copy(dest.c, src.c, sizeof(char) * NUMBER_OF_ELEMENTS + 1);
        printf("%s\n", dest.c);
        break;
    case to_int:
        fill_int_array(src.i, &argv[2], NUMBER_OF_ELEMENTS);
        memory_copy(dest.i, src.i, sizeof(int) * NUMBER_OF_ELEMENTS);
        print_int_array(dest.i, NUMBER_OF_ELEMENTS);
        break;
    case to_dbl:
        fill_dbl_array(src.d, &argv[2], NUMBER_OF_ELEMENTS);
        memory_copy(dest.d, src.d, sizeof(double) * NUMBER_OF_ELEMENTS);
